...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!canEditEmployee(auth.user.role)) {
    return NextResponse.json({ message: "Perfil sem permissao para desativar colaboradores." }, { status: 403 });
  }
  ...
  return NextResponse.json({ message: "Colaborador desativado com sucesso.", employee });
}
